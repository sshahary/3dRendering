...
static inline tmx::mat4 makeRotationAxisAngle(float deg, tmx::vec3 axis){
    ...
    if (len == 0.f) return tmx::mat4::identity();
...
void Transform::setRotation(float degrees, const tmx::vec3& axis){ angleDeg_ = degrees; axis_ = axis; dirty_ = true; }
void Transform::setScale(const tmx::vec3& s){ S_ = s; dirty_ = true; }
void Transform::setPivot(const tmx::vec3& p){ pivot_ = p; dirty_ = true; }
...
void Transform::rebuild() const {
    ...
    const tmx::mat4 Tpinv = tmx::mat4::translation(-pivot_.x, -pivot_.y, -pivot_.z);
