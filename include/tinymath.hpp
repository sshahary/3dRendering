...
inline mat4 operator*(const mat4 &a, const mat4 &b) {
  ...
  for (int c = 0; c < 4; ++c)
    for (int r_i = 0; r_i < 4; ++r_i) {
      r.m[c * 4 + r_i] = a.m[0 * 4 + r_i] * b.m[c * 4 + 0] +
...
inline mat4 lookAt(const vec3 &eye, const vec3 &target, const vec3 &up) {
  vec3 cz =
      normalize(vec3{eye.x - target.x, eye.y - target.y, eye.z - target.z});
  vec3 cx = normalize(vec3{up.y * cz.z - up.z * cz.y, up.z * cz.x - up.x * cz.z,
                           up.x * cz.y - up.y * cz.x});
  vec3 cy = vec3{cz.y * cx.z - cz.z * cx.y, cz.z * cx.x - cz.x * cx.z,
                 cz.x * cx.y - cz.y * cx.x};
