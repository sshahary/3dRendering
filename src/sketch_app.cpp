...
// --- Helper: clip against near plane (z + w >= 0) and NDC box, then map to
// screen ---
static inline bool clipNearAndNDCToScreen(const tmx::vec4 &A,
                                          const tmx::vec4 &B, int W, int H,
                                          tmx::ivec2 &SA, tmx::ivec2 &SB) {
  auto insideNear = [](const tmx::vec4 &p) {
    return (p.z + p.w) >= 0.0f;
  }; // z >= -w
  ...
  if (a_in ^ b_in) {
    ...
    tmx::vec4 I{A.x + t * (B.x - A.x), A.y + t * (B.y - A.y),
  ...
  if (std::fabs(a.w) < epsw || std::fabs(b.w) < epsw)
    return false;
...
static inline void fillDepthTri(const tmx::ivec2 &a, float za,
                                ...
                                const tmx::ivec2 &c, float zc, int W, int H,
...
tmx::vec3 SketchApp::computeCenter(const std::vector<tmx::vec3> &v) {
  ...
  return tmx::vec3{(mn.x + mx.x) * 0.5f, (mn.y + mx.y) * 0.5f,
                   (mn.z + mx.z) * 0.5f};
}

SketchApp::SketchApp(const std::vector<tmx::vec3> &positions,
                     const std::vector<std::pair<int, int>> &edges, int W,
                     int H)
    : W_(W), H_(H), center_(computeCenter(positions)),
      ...
      pipe_(M_, V_, P_), vp_(W, H), rast_(W, H), pos_(positions),
      edges_(edges) {
...
void SketchApp::clearExplicitCamera() { explicit_cam_ = false; }
...
const std::vector<uint32_t> &SketchApp::render() {
  ...
  // keep yaw bounded
  if (yaw_deg_ > 360.f)
    yaw_deg_ = std::fmod(yaw_deg_, 360.f);
  if (yaw_deg_ < -360.f)
    yaw_deg_ = std::fmod(yaw_deg_, 360.f);
  ...
  if (explicit_cam_) {
    ...
    const tmx::vec3 eye{std::cos(cp) * std::cos(cy) * dist_,
                        std::sin(cp) * dist_,
                        std::cos(cp) * std::sin(cy) * dist_};
    const tmx::vec3 up =
        (std::cos(cp) >= 0.0f) ? tmx::vec3{0, 1, 0} : tmx::vec3{0, -1, 0};
  ...
  // clip space vertices
  pipe_.transformIntoParallel(pos_, clip_,
  ...
  size_t added = 0;
  for (size_t i = 0; i < edges_.size() && added < max_edges_;
       i += edge_stride_) {
  ...
  auto worker = [&](int start, int end) {
    for (int ti = start; ti < end; ++ti) {
      for (int idx : tiles[ti].e) {
        ...
        tmx::ivec2 A, B;
        if (!clipNearAndNDCToScreen(clip_[e.first], clip_[e.second], W_, H_, A,
                                    B))
          continue;
