...
static bool save_png_argb32(const std::string& path, int W, int H, const std::vector<uint32_t>& argb)
{
    ...
    for (int y = 0; y < H; ++y) {
        std::memcpy(img.scanLine(y), reinterpret_cast<const uint8_t*>(argb.data()) + size_t(y) * W * 4, size_t(W) * 4);
    }
...
int main(int argc, char** argv)
{
    ...
    std::transform(proj.begin(), proj.end(), proj.begin(), [](unsigned char c){ return std::tolower(c); });
    const bool useOrtho =
    ...
    int W = 1000, H = 1000;
    if (const char* w = std::getenv("RTF_WIDTH"))  W = std::max(16, std::atoi(w));
    if (const char* h = std::getenv("RTF_HEIGHT")) H = std::max(16, std::atoi(h));
