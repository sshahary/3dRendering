...
const float fov_rad = 60.0f * 3.1415926535f / 180.0f;      // matches simplePerspective()
...
const std::vector<uint32_t>& SketchApp::render(){
    ...
    // Flip up past the poles so lookAt never hits a singularity
    ...
    par::parallel_for(0, clip_.size(), [&](std::size_t i){
        ...
        float nz = p.z * iw;
        float nz01 = nz * 0.5f + 0.5f;
        clip_[i].z = nz01;
    
        uint8_t c=0;
        if(nx < -1.f) c|=1; if(nx > 1.f) c|=2;
        if(ny < -1.f) c|=4; if(ny > 1.f) c|=8;
        if(nz01 < 0.f) c|=16; if(nz01 > 1.f) c|=32;
        codes_[i]=c;
    
        int x = int((nx*0.5f+0.5f)*float(W_));
        int y = int((1.0f-(ny*0.5f+0.5f))*float(H_));
        scr_[i] = {x,y};
    ...
    if (faces_ && !faces_->empty()) {
        clearDepth();
    // Triangulate faces into depth buffer (fan)
    ...
    if ((codes_[e.first] & codes_[e.second]) != 0) continue;
