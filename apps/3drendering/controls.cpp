...
void SketchWidget::paintEvent(QPaintEvent *) {
  ...
  QImage img(reinterpret_cast<const uchar *>(px.data()), width(), height(),
...
void SketchWidget::mouseReleaseEvent(QMouseEvent *) { drag_ = false; }
...
void SketchWidget::keyPressEvent(QKeyEvent *e) {
  ...
  if (e->key() == Qt::Key_C) {
    bool okX, okY, okZ;
    double x = QInputDialog::getDouble(this, "Camera X", "x:", -4.0, -1e6, 1e6,
                                       2, &okX);
    if (!okX)
      return;
    double y = QInputDialog::getDouble(this, "Camera Y", "y:", -3.0, -1e6, 1e6,
                                       2, &okY);
    if (!okY)
      return;
    double z = QInputDialog::getDouble(this, "Camera Z", "z:", 2.0, -1e6, 1e6,
                                       2, &okZ);
    if (!okZ)
      return;
